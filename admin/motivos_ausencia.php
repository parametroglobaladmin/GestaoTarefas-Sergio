...
<div class="container">
    <?php if (!empty($mensagem)): ?>
        <div class="toast"><?= htmlspecialchars($mensagem) ?></div>
    <?php elseif (!empty($erro)): ?>
        <div class="toast erro"><?= htmlspecialchars($erro) ?></div>
    <?php endif; ?>

    <h1>Gestão de Motivos de Ausência</h1>

    <div class="top-buttons">
        <button onclick="abrirModalNovoMotivo()" class="botao-modal">Novo Motivo</button>
        <a href="painel.php" class="botao-modal">Voltar</a>
    </div>
    ...
    <table class="tabela">
        ...
        <?php if (empty($motivos)): ?>
            <tr><td colspan="3">Nenhuma motivo registado.</td></tr>
        <?php else: ?>
            <?php foreach ($motivos as $motivo): ?>
                <tr>
                    ...
                    <td>
                        <a href="#" class="botao-modal" onclick="abrirModalEditar('<?= $motivo['id'] ?>', '<?= htmlspecialchars($motivo['codigo'], ENT_QUOTES) ?>', '<?= htmlspecialchars($motivo['descricao'], ENT_QUOTES) ?>')">Editar</a>
                        <a href="#" class="botao-modal" style="background:#c0392b; color:white;" onclick="abrirModalEliminar(<?= $motivo['id'] ?>)">Eliminar</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        <?php endif; ?>
...
<script>
    ...
    function abrirModalEditar(id, codigo, descricao) {
